...
#        imageList12.append(img_flip2)
    ...
    with torch.no_grad():
        ...
        imgDataTensor = torch.from_numpy(input_data).type(torch.FloatTensor).to(device)
        # imgDataTensor_light = torch.from_numpy(input_data_light).type(torch.FloatTensor).cuda()
        imgDataTensor_light = torch.from_numpy(input_data_light).type(torch.FloatTensor).to(device)
